...
export const LoanTracker: React.FC<LoanTrackerProps> = ({
  ...
  const handleSendWhatsAppReminder = async (tx: POSTransaction) => {
    triggerHaptic('light');
    const meta = getLoanMeta(tx);
    let rawPhone = getLoanCustomerPhone(tx).replace(/[^0-9+]/g, '');
    if (!rawPhone) {
      const entered = await customPrompt("Enter customer WhatsApp phone number (e.g. 255768...):", "255", "WhatsApp Phone Number");
      if (!entered) return;
