...
import { Search, ShoppingCart, User, Menu, ChevronDown, ShoppingBag, Star, TrendingUp, Sparkles, Tag, X, SlidersHorizontal, ShieldCheck, ArrowRight, Globe, Sun, Moon, Lock, Zap, Monitor, Download, ExternalLink, SearchX, RotateCw, Bot } from 'lucide-react';
        ...
        <div>
          ...
          {matchingProducts.length === 0 ? (
            <div className="py-6 px-3 text-center">
              <div className={`w-11 h-11 mx-auto mb-3 rounded-xl flex items-center justify-center border ${
                isDark ? 'bg-slate-800/80 text-amber-400 border-slate-700' : 'bg-amber-50 text-amber-600 border-amber-200/80'
              }`}>
                <SearchX className="w-5 h-5" />
              </div>
              <p className={`text-xs font-bold mb-1 ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>
                {language === 'sw' ? `Hakuna bidhaa iliyopatikana: "${searchTerm}"` : `No products found for "${searchTerm}"`}
              </p>
              <p className={`text-[11px] mb-3.5 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                {language === 'sw' ? 'Jaribu maneno mengine au chagua mada maarufu hapa chini:' : 'Check spelling or try popular categories below:'}
              </p>

              {/* Quick Pills */}
              <div className="flex flex-wrap items-center justify-center gap-1.5 mb-3">
                {['Subwoofer', 'Hisense', 'Samsung', 'TV', 'Fridge'].map((pill) => (
                  <button
                    key={pill}
                    type="button"
                    onMouseDown={(e) => {
