...
export const VisitorActivityHeatmap: React.FC<VisitorActivityHeatmapProps> = ({
  ...
  // Accurate East Africa Time calculation
  const clientEatDate = new Date(Date.now() + 3 * 60 * 60 * 1000);
  const clientUtcDay = clientEatDate.getUTCDay(); // 0 = Sun, 1 = Mon ... 6 = Sat
  const fallbackTodayIndex = (clientUtcDay + 6) % 7; // 0 = Mon, 1 = Tue, 2 = Wed, 3 = Thu, 4 = Fri, 5 = Sat, 6 = Sun
  const todayDayIndex = heatmapData?.currentDayIndex !== undefined ? heatmapData.currentDayIndex : fallbackTodayIndex;
  const todayDayName = heatmapData?.currentDayName || (todayDayIndex === 5 ? 'Saturday' : todayDayIndex === 4 ? 'Friday' : DAYS_SHORT[todayDayIndex]);
  const busiestDayIndex = heatmapData?.busiestDayIndex !== undefined ? heatmapData.busiestDayIndex : 4;
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-bold border ${
                  ...
                  Today: {todayDayName} (EAT)
                </span>
                <span className={`inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-bold border ${
                  isDark ? 'bg-indigo-950/60 text-indigo-300 border-indigo-800/60' : 'bg-indigo-50 text-indigo-700 border-indigo-200'
                }`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className={`p-4 rounded-2xl border transition-all ${
          ...
          <div className={`text-base font-black truncate ${isDark ? 'text-white' : 'text-slate-900'}`}>
            {peakDay} at {peakHour} <span className="text-xs font-semibold text-indigo-400 font-normal">(Historical Peak)</span>
          </div>
          <p className={`text-xs mt-1.5 leading-relaxed ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
            <span className="font-semibold text-emerald-400">Current Day:</span> {todayDayName} (EAT). Launch flash sales & WhatsApp campaign broadcasts during <strong>{recommendedPromo}</strong>.
          </p>
        </div>
        ...
        <div className="overflow-x-auto pb-2 pt-1">
          <div className="min-w-[760px] space-y-1">
            ...
            {DAYS_SHORT.map((dayName, dayIndex) => {
              ...
              const isPeakDay = dayIndex === busiestDayIndex;

              return (
                <div key={`day-row-${dayName}`} className="grid grid-cols-[68px_repeat(24,1fr)] gap-1 items-center">
                  {/* Day Label with Clear Role Badge */}
                  <div className={`text-xs font-black tracking-tight flex items-center gap-1 ${
                    ...
                    <span>{dayName}</span>
                    {isToday && (
                      <span className="inline-flex items-center px-1 py-0.2 rounded text-[9px] font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-500/30" title="Today in East Africa Time (Current active day)">
                        • Today
                      </span>
                    )}
                    {!isToday && dayIndex === 6 && (
                      <span className="text-[9px] font-normal text-slate-500 dark:text-slate-400" title="Past Sunday within the selected analysis window (not tomorrow)">
                        (Past)
                      </span>
                    )}
                    {!isToday && isPeakDay && dayIndex !== 6 && (
                      <span className="text-[10px]" title="Historical Peak Day">👑</span>
                    )}
                  </div>
          ...
          <div className="lg:col-span-4 space-y-2">
            ...
            <div className="space-y-1.5">
              {heatmapData.dailyDistribution?.map((dItem) => {
                ...
                const isTopDay = dItem.dayIndex === busiestDayIndex;
                const isToday = dItem.dayIndex === todayDayIndex;

                return (
                  <div key={`d-rank-${dItem.dayIndex}`} className="space-y-0.5">
                    <div className="flex items-center justify-between text-xs">
                      <div className="flex items-center gap-1.5">
                        <span className={`font-bold ${isToday ? 'text-emerald-400' : isTopDay ? 'text-indigo-400' : isDark ? 'text-slate-300' : 'text-slate-700'}`}>
                          {dItem.dayName}
                        </span>
                        {!isToday && dItem.dayIndex === 6 && (
                    ...
                    <div className={`h-1.5 w-full rounded-full overflow-hidden ${isDark ? 'bg-slate-800' : 'bg-slate-200'}`}>
                      <div 
                        style={{ width: `${pct}%` }} 
                        className={`h-full rounded-full ${isToday ? 'bg-emerald-500' : isTopDay ? 'bg-indigo-500' : isDark ? 'bg-slate-600' : 'bg-slate-400'}`}
                      ></div>
                    </div>
                  </div>
                );
