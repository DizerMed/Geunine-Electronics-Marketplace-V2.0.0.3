...
/**
 * Send real-time presence heartbeat ping to server
 */
export async function sendVisitorHeartbeat(options?: { isAdmin?: boolean }): Promise<{ liveOnlineNow?: number; liveCustomersOnline?: number } | null> {
  if (typeof window === 'undefined') return null;
  try {
    const visitorId = getOrCreateVisitorId();
    const sessionId = getOrCreateSessionId();
    const { deviceType, browser, os } = getDeviceContext();
    const userId = sessionStorage.getItem('ge_auth_user_id') || undefined;
    const userEmail = sessionStorage.getItem('ge_auth_user_email') || undefined;
    const userName = sessionStorage.getItem('ge_auth_user_name') || undefined;
...
/**
 * Send real-time leave beacon on tab unload or hide
 */
export function sendVisitorDeparture(): void {
  if (typeof window === 'undefined') return;
  try {
    const visitorId = getOrCreateVisitorId();
...
/**
 * Initialize visitor presence heartbeat lifecycle (every 25 seconds while tab is active)
 */
export function startVisitorPresenceHeartbeat(options?: { isAdmin?: boolean }): () => void {
  if (typeof window === 'undefined') return () => {};

  // Immediate initial heartbeat ping
  sendVisitorHeartbeat(options);
 ...
 */
export async function fetchVisitorSummary(timeframe: string = '30days', startDate?: string, endDate?: string): Promise<VisitorAnalyticsSummary> {
